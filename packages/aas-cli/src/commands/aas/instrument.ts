...
export class InstrumentCommand extends AasCommand {
  ...
  private sourceCodeIntegration = Option.Boolean('--source-code-integration,--sourceCodeIntegration', false, {
    description:
      'Enable source code integration to add git metadata as tags. Defaults to disabled. Specify `--source-code-integration` to enable.',
  })

  private uploadGitMetadata = Option.Boolean('--upload-git-metadata,--uploadGitMetadata', false, {
    description: 'Upload git metadata to Datadog. Defaults to disabled. Specify `--upload-git-metadata` to enable.',
  })
